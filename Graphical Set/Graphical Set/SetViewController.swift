...
class SetViewController: UIViewController, didTouchCard {
    ...
    @objc private func configureView() {
        grid = Grid(layout: .aspectRatio(CGFloat(0.8)), frame: drawView.frame)
        grid.cellCount = set.cardsInPlay.count
        if cardViews.count > 0 {cardViews.removeAll(); cardViews.forEach {$0.removeFromSuperview()}}
        for card in set.cardsInPlay.indices {
            ...
            if set.selectedCards.contains(set.cardsInPlay[card]) {cardView.layer.borderWidth = 3.0 ; cardView.layer.borderColor = #colorLiteral(red: 0.2392156869, green: 0.6745098233, blue: 0.9686274529, alpha: 1)}
    ...
    func cardWasSelected(at cardIndex: Int) {
        let selectedCard = set.cardsInPlay[cardIndex]
        if set.selectedCards.contains(selectedCard) {
            // card was already selected and i need to clear it out 
            guard let selectedCardindex = selectedCards.firstIndex(of: cardIndex) else {return}
            selectedCards.remove(at: selectedCardindex)
            set.unSelectCard(selectedCard)
